...
int main() {
  ...
  */
	...
	freopen("unfairplay.in", "r", stdin);

	int pts[101]; ii game[1001];
	int V, E;
	while ((cin>>V), (V!=-1)) {
		cin>>E;
		max_flow mf(E+V+1);
		for (int i=1;i<=V;++i) cin>>pts[i];
		int in=0;
		for (int i=1;i<=E;++i) {
			auto &[u,v]=game[i]; cin>>u>>v;
			if (u==V or v==V) {
				pts[V]+=2; continue;
			}
			mf.add_edge(i, E+u, 1e9);
			mf.add_edge(i, E+v, 1e9);
			mf.add_edge(0, i, 2);
			in += 2;
		}
		int out=0;
		for (int i=1;i<V;++i) {
			if (pts[V] <= pts[i]) { cout<<"NO\n"; goto end; }
			int w = pts[V] - pts[i] - 1;
			mf.add_edge(E+i, E+V, w);
			out+=w;
		}
		if (in > out) { cout<<"NO\n"; continue; }
		if (mf.dinic(0, E+V) != in) { cout<<"NO\n"; continue; }
		for (int i=1;i<=E;++i) {
			auto &[u,v]=game[i];
			if (u==V) cout<<0;
			else if (v==V) cout<<2;
			else {
				auto &[vv,cap,f] = mf.EL[mf.AL[i][1]];
				cout<<f;
			}
			cout<<" ";
		} cout<<"\n";
