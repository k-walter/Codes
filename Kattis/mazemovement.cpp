...
public:
  ...
  void solve(int s, int t) {
    dinic(s,t);
    vi q = {s};
      for (auto &[v,cap,flow]:EL)
        printf("%d %d %d\n", v,cap,flow);
      return;
    vector<bool> vis(AL.size(), 0); vis[s]=1;
    for (int i=0;i<q.size();++i) {
      int &u = q[i];
      for (int &j: AL[u]) {
        auto &[v,cap,flow] = EL[j];
        if (vis[v]) continue;
        if (cap == abs(flow)) continue;
        q.push_back(v); vis[v]=1;
      }
    }
    printf("%d\n",q.size());
    for (int &u:q) printf("%d\n",u);
  }
};
...
int main() {
  ...
  */
  ...
  // freopen("mazemovement.in", "r", stdin);

  int V; cin>>V; V+=2;
  max_flow mf(V);
  int vtx[V];
  int s,t,hi=0,lo=INT_MAX;
  for (int u=1;u<V-1;++u) {
    cin>>vtx[u];
    for (int v=1;v<u;++v) {
      int w = gcd(vtx[u], vtx[v]);
      if (w == 1) continue;
      mf.add_edge(u, v, w, 0);
    }
    if (vtx[u]<lo) { s=u; lo=vtx[u]; }
    else if (vtx[u]>hi) { t=u; hi=vtx[u]; }
  }
  mf.add_edge(0,s,1e9);
  mf.add_edge(t,V-1,1e9);

  cout<<mf.edmonds_karp(0,V-1);
